...
class MultiRoundChatWorkload(BaseWorkload):
    ...
    async def warmup(self) -> None:
        """Send one warmup request per session with ``max_tokens=2``.

        Uses ``max_tokens=2`` instead of 1 to ensure compatibility with
        disaggregated prefill-decode routers that consume the first token
