...
class SageMakerHyperPodConnector(RemoteConnector):
    ...
    def __init__(
        ...
        **kwargs,  # Accept and ignore unused legacy parameters
        ...
        Args:
            sagemaker_hyperpod_url: Base URL of the KVServiceSM daemon
    ...
    def _init_shared_memory(self):
        """Initialize shared memory connection to KVServiceSM daemon."""
        try:
            ...
            logger.error(
                ...
                "Ensure KVServiceSM daemon is running."
            )
    ...
    async def put(self, key: CacheEngineKey, memory_obj: MemoryObj):
        """Store data to KVServiceSM (queued with priority)."""
        return await self.pq_executor.submit_job(
            self._put,
            key=key,
    ...
    async def list(self) -> List[str]:
        """List operation not supported by KVServiceSM."""
        return []

    def remove_sync(self, key: CacheEngineKey) -> bool:
        """Remove operation not supported by KVServiceSM."""
        return True
    ...
    async def close(self):
        ...
        if self.shared_memory_map is not None:
            try:
                ...
                logger.warning(f"Error releasing shared memory map: {e}")
            self.shared_memory_map = None
